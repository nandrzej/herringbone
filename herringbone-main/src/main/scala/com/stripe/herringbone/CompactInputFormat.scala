...
class CompactInputFormat[T](readSupportClass: Class[_ <: ReadSupport[T]])
    extends ParquetInputFormat[T](readSupportClass) {

  // default HDFS block size
  val DEFAULT_TARGET = 256 * 1024 * 1024 // 256MB
  ...
  override def getSplits(context: JobContext): JavaList[InputSplit] = {
    ...
    val m =
      if (splits.isEmpty) splits
        ...
        mergeSplits(splits,
                    conf.getInt("mapreduce.input.fileinputformat.split.maxsize", DEFAULT_TARGET))
    m.asInstanceOf[List[InputSplit]].asJava
  }

  def mergeSplits(splits: List[ParquetInputSplit],
                  maxSplitSzie: Int): List[MergedInputSplit] = {
    ...
    var buckets =
      MutableList[MutableList[ParquetInputSplit]](MutableList(splits.head))
    splits.tail.foreach { split =>
      val bucket = buckets.minBy { b =>
        b.map { _.getLength }.sum
      }
      if ((split.getLength + bucket.map { _.getLength }.sum) < maxSplitSzie) {
        bucket += split
      } else {
    ...
    buckets.map { b =>
      new MergedInputSplit(b.toList)
    }.toList
  }

  override def createRecordReader(
      split: InputSplit,
      context: TaskAttemptContext): MergedRecordReader[T] = {
    val readSupport = ParquetInputFormat.getReadSupportInstance[T](
      ContextUtil.getConfiguration(context))
    split match {
      case s: MergedInputSplit =>
        new MergedRecordReader[T](s, context, readSupport)
      case _ =>
        throw new Exception(s"Expected a MergedInputSplit. Found a $split.")
    }
  }
...
class MergedInputSplit(var splits: List[ParquetInputSplit])
    extends InputSplit
    with Writable {
  def this() = this(List())
  ...
  override def getLocations: Array[String] =
    splits.flatMap { _.getLocations }.toArray
...
class MergedRecordReader[T](split: MergedInputSplit,
    ...
    extends ParquetRecordReader[T](readSupport) {
  ...
  def startNextSplit(split: MergedInputSplit,
                     context: TaskAttemptContext): Boolean = {
...
class CompactGroupInputFormat
    extends CompactInputFormat[Group](classOf[GroupReadSupport]) {}
