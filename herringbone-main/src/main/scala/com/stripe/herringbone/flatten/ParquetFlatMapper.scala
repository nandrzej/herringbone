...
abstract class ParquetFlatMapper[ValueOut]
    extends Mapper[Void, Group, Void, ValueOut] {
  ...
  override def setup(context: Mapper[Void, Group, Void, ValueOut]#Context) {
    ...
    flattenedSchema = MessageTypeParser.parseMessageType(
      context.getConfiguration.get("parquet.example.schema"))
  ...
  override def map(key: Void,
                   value: Group,
                   context: Mapper[Void, Group, Void, ValueOut]#Context) {
