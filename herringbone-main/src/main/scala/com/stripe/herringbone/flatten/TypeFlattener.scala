...
class TypeFlattener(separator: String, renameId: Boolean)
    extends TypeConverter[List[Type]] {
  def convertPrimitiveType(path: JList[GroupType],
    ...
    val types =
      if (TypeFlattener.omitIdField(primitiveType.getName, path.size, renameId))
        path.asScala.tail
      else
    ...
    List(
      new PrimitiveType(Type.Repetition.OPTIONAL,
                        typeName,
                        primitiveType.getTypeLength,
                        name))
  }

  def convertGroupType(path: JList[GroupType],
                       groupType: GroupType,
                       children: JList[List[Type]]) = {
  ...
  def convertMessageType(messageType: MessageType,
                         children: JList[List[Type]]) = flatten(children)
...
object TypeFlattener {
  def flatten(messageType: MessageType,
              ...
              renameId: Boolean) = {
    val flattened =
      messageType.convertWith(new TypeFlattener(separator, renameId))
  ...
  def omitIdField(fieldName: String,
