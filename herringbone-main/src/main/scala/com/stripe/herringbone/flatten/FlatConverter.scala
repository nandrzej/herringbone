...
object FlatConverter {
  def groupToTSV(group: Group,
                 flatSchema: MessageType,
                 separator: String,
                 renameId: Boolean): String = {
    ...
    val fieldValues = (0 until flatSchema.getFieldCount).map { field =>
      ...
      if (valueCount == 0) {
        ...
        escapeString(flatGroup.getValueToString(field, 0))
        System.err.println(
          "Warning: Field contains multiple values, extracting only the first")
  ...
  def flattenGroup(group: Group,
                   flatSchema: MessageType,
                   separator: String,
                   renameId: Boolean) = {
    var flatGroup = new SimpleGroup(flatSchema)
    val writer = new GroupWriter(
      new FlatConsumer(flatGroup, separator, renameId),
      group.getType)
    writer.write(group)
    flatGroup
  }
